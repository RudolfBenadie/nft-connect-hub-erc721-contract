...
async function main() {
  ...
  if (CollectionConfig.whitelistAddresses.length < 1) {
    throw '\x1b[31merror\x1b[0m ' + 'The restricted list is empty, please add some addresses to the configuration.';
  }
  ...
  const restrictedSalePrice = utils.parseEther(CollectionConfig.restrictedSale.price.toString());
  if (!await (await contract.cost()).eq(restrictedSalePrice)) {
    console.log(`Updating the token price to ${CollectionConfig.restrictedSale.price} ${CollectionConfig.mainnet.symbol}...`);
  ...
  if (!await (await contract.maxMintAmountPerTx()).eq(CollectionConfig.restrictedSale.maxMintAmountPerTx)) {
    console.log(`Updating the max mint amount per TX to ${CollectionConfig.restrictedSale.maxMintAmountPerTx}...`);
  ...
  if (!await contract.restrictedMintEnabled()) {
    console.log('Enabling restricted sale...');

    await (await contract.setRestrictedMintEnabled(true)).wait();
  }

  console.log('Restricted sale has been enabled!');
}
