...
enum SaleType {
  WHITELIST = CollectionConfig.restrictedSale.price,
...
describe(CollectionConfig.contractName, function () {
  ...
  it('Check initial data', async function () {
    ...
    expect(await contract.maxMintAmountPerTx()).to.equal(CollectionConfig.restrictedSale.maxMintAmountPerTx);
    expect(await contract.hiddenMetadataUri()).to.equal(CollectionConfig.hiddenMetadataUri);

    expect(await contract.paused()).to.equal(true);
    expect(await contract.restrictedMintEnabled()).to.equal(false);
  ...
  it('Before any sale', async function () {
    ...
    await expect(contract.connect(whitelistedUser).whitelistMint(1, [], { value: getPrice(SaleType.WHITELIST, 1) })).to.be.revertedWith('The restricted sale is not enabled!');
    ...
    await expect(contract.connect(holder).whitelistMint(1, [], { value: getPrice(SaleType.WHITELIST, 1) })).to.be.revertedWith('The restricted sale is not enabled!');
    await expect(contract.connect(owner).mint(1, { value: getPrice(SaleType.WHITELIST, 1) })).to.be.revertedWith('The contract is paused!');
    await expect(contract.connect(owner).whitelistMint(1, [], { value: getPrice(SaleType.WHITELIST, 1) })).to.be.revertedWith('The restricted sale is not enabled!');
  ...
  it('Restricted list sale', async function () {
    // Build MerkleTree
    ...
    await contract.setrestrictedMintEnabled(true);
    ...
    // Pause restricted sale
    await contract.setrestrictedMintEnabled(false);
  ...
  it('Pre-sale (same as public sale)', async function () {
    ...
    // Sending a restricted mint transaction
    await expect(contract.connect(whitelistedUser).whitelistMint(
      1,
      [],
      { value: getPrice(SaleType.WHITELIST, 1) },
  ...
  it('Owner only functions', async function () {
    ...
    await expect(contract.connect(externalUser).setrestrictedMintEnabled(false)).to.be.revertedWith('Ownable: caller is not the owner');
